xpackage com.example.test.product.service;
...
/**
 * 상품 등록 및 목록·상세 조회·검색 처리 담당.
 * 인증 정보 추출은 호출하는 쪽에서 처리한다.
 *
...
public class ProductService {
     ...
     * TODO: 내 상품 Controller 및 JWT 연결.
     */
    @Transactional(readOnly = true)
    public List<ProductListResponse> getMyProducts(
    ...
    /**
     * 검색어 앞뒤 공백을 제거하고 상품명을 부분 검색한다.
     * 결과가 없으면 빈 목록을 반환한다.
     * TODO: 검색 Controller 및 검증 오류 응답 연결.
     */
    @Transactional(readOnly = true)
    public List<ProductListResponse> searchProducts(
            @NotNull @Valid ProductSearchRequest request
    ) {
    ...
    /**
     * 상품 ID로 한 건을 조회한다.
     * 존재하지 않으면 ProductNotFoundException 발생.
    ...
    /**
     * 전체 상품을 등록 시간 내림차순으로 조회한다.
     * 같은 시간이면 상품 ID 내림차순.
     * TODO: 목록 Controller 연결.
     */
    @Transactional(readOnly = true)
    ...
    public Long createProduct(
            @NotNull @Valid ProductCreateRequest request,
            @NotNull @Positive Long userId
    ) {
